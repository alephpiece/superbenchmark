...
+ `proc_rank`
+ `proc_num`
+ `gpu_numa_node`: NUMA node of the GPU associated with `proc_rank`, available in `local` mode and evaluated on the target node when supported by the platform topology utility.
