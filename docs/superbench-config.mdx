...
`numactl` binding to use in `local` mode. This is inserted by the runner after `prefix` and before the benchmark command.
...
`cpunodebind` and `membind` can be explicit NUMA node values, formatted strings using `proc_rank` and `proc_num`, or `gpu_affinity`. GPU-relative values are evaluated on the target node through `sb node topo --get gpu-numa-affinity --gpu-id`.
